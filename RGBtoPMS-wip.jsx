...
$('#colorPallete').colpick({
	...
	$( ".sourceCodeOptionDiv input" ).keyup(function() {
		performPMS();
	});
...
var view = $("#viewName").val().split("-")[0];
	...
	if(view == "hex"){
		...
		$("#hiddenHex").val(rgbToHex($("#r").val(),$("#g").val(),$("#b").val()));
		convertColor();
	}
	else if(view == "hsv"){
		...
		$("#hiddenHex").val(rgbToHex(rgb[0],rgb[1],rgb[2]));
		convertColor();
	}
	else if(view == "cmyk"){
...
function convertColor(){
	var hex = $("#hiddenHex").val();
	
	if(hex.trim().length != 6){
		return false;
	}
	...
	if (aww<=400){ bpl=5;}
	else if (aww<500){ bpl=6;}
	else if (aww<600){ bpl=7;}
	else if (aww<800){ bpl=8;}
	else if (aww<1000){ bpl=10;}
	else { bpl=12;}
	    ...
	    if ((m.length) > 0){
	      ...
	      var ipms = 0;
	      var mtr = Math.ceil(m.length / bpl);
	      for(var i=0; i < mtr ;i++){
	      m2 += '<tr align="center">';   
	      for(var j=0; j < bpl ;j++){
	      m2 = m2 + '<td><div>' ;
	      ...
	      m2 = m2 + m[ipms] + '</div><div title="' + rgbcode + '" style="background-color:#' + rgbcode + ';height:100px;width:100px;">&nbsp;';
	      ipms = ipms + 1;   
	      } 
	      m2 = m2 + '</div></td>';   
	      }
